...
if [ ! $(which chef-server-ctl) ]; then
  ...
  chef-server-ctl user-create chefadmin Chef Admin admin@foobar.local insecurepassword --filename chef-admin.pem
  chef-server-ctl org-create foobar "Foo Bar, Inc." --association_user chefadmin --filename foobar-validator.pem
fi
